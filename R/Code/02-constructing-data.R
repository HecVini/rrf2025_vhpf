...
acre_conv <- 2.47
usd <- 0.00037 # 1 TZS = 0.00037 USD (2020)
...
hh_data %>% filter(is.na(ar_unit)) %>% subset(select = c(vid, hhid, ar_farm, ar_unit, area_acre))
...
hh_mem_collapsed <- mem_data %>%
    ...
    mutate(treat_cost_usd = if_else(is.na(treat_cost_usd), NA_real_,
                            mean(treat_cost_usd, na.rm = TRUE)))
hh_mem_collapsed <- hh_mem_collapsed %>%
